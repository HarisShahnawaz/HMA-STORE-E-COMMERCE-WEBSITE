...
const CATEGORY_MAP = {
  All: () => true,
  Tops: (p) => /polo|shirt|tee|hoodie|sweater|blouse|top|cardigan/i.test(p.name),
  Bottoms: (p) => /trouser|chino|jean|pant|bottom|skirt|legging/i.test(p.name),
  Dresses: (p) => /dress|gown|suit|set|jumpsuit/i.test(p.name),
  Outerwear: (p) => /jacket|coat|blazer|vest|puffer|trench/i.test(p.name),
  Accessories: (p) => /belt|scarf|bag|earring|jewelry|accessory/i.test(p.name),
};
...
export default function Women() {
  ...
  const filtered = useMemo(() => {
    ...
    return result;
  }, [womenProducts, selectedCategory, priceMax, showAiOnly, sortValue]);
  ...
  return (
    <div className="bg-white min-h-screen pt-20">
      <div className="max-w-360 mx-auto px-4 md:px-8">
        ...
        <CollectionHeader
          title="Women"
          description="Elegant silhouettes and contemporary designs. Explore our curated selection of women's fashion, from timeless staples to bold statement pieces."
